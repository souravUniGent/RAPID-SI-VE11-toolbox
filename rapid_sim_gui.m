...
for ll=1:1
    ...
    Rx=1;Ry=1;rp=2;
    [sub_data_part,scale_half,x1,x2,AS,Fs,blip_pt,tot_sig,sub_signal]=simulation_test_17_2();
    spectralwidth=Fs;
    PE=x1;
    RE=x2;
    ...
    flag=0;
    signal=(sub_signal');
    fac2=size(signal,2)./PE;
    for kk=0:size(signal,1)-1
        for lll=0:AS-1
            for l=1:PE/AS
                 sub_data(:,l+(kk*PE)+(lll*PE/AS)) =  signal(kk+1,(1 + 1*fac2*(l-1) +(fac2*lll*PE/AS)):(1*fac2*l +(fac2*lll*PE/AS)));
                 temp=sub_data(:,l+(kk*PE)+(lll*PE/AS));
                 temp(1:blip_pt)=[];
                 sub_data_part(:,l+(kk*PE)+(lll*PE/AS))=(temp);
        %          sub_data_ref(:,l+(kk*PE)) =  reference(kk+1,1 + 1*fac2*(l-1):1*fac2*l);
        %          temp_ref=sub_data_ref(:,l+(kk*PE));
        %          temp_ref(1:blip_pt)=[];
        %          sub_data_part_ref(:,l+(kk*PE))=temp_ref;
            end
        end
    end
    water_supress=2;
...
%                [fit_sig,amp,Ind_sig,freq] = HSVDPK_new(sub_data_part(:,i),fres,Fs,5) ;
%                 ind=find((freq>=-50 & freq<=50));
%                 half_sig2=sum(Ind_sig(:,ind),2);
%                 ind2=find((freq>=-800 & freq<=-350));
%                 half_sig3=sum(Ind_sig(:,ind2),2);
...
%     freq_tempo2=freq;
%     freq=freq_tempo
%     fres_sub=(-( spectralwidth)/2 + (( spectralwidth)/(length(conj(sub_data_part(:,PE/2 +1+1*(RE*PE*0.5))))*1))*(0:((length(conj(sub_data_part(:,PE/2 +1+1*(RE*PE*0.5))))*1-1))));
%     [fit_sig_tempo,amp_tempo,Ind_sig_tempo,freq_tempo,Z_sig_tempo,FWHM_tempo] = HSVDPK_new(conj(sub_data_part(:,PE/2 +1+1*(RE*PE*0.5))),fres_sub,Fs,model_order) ;
%     ind=find((freq>=-700 & freq<=100));
%     model_order=length(ind);
%     freq_tempo=freq_tempo(ind); FWHM_tempo=FWHM_tempo(ind);
    for i=1:model_order
%         Z_sig(:,i)=exp((1i*2*pi*(freq(i))).*time_sample);
        Z_sig(:,i)=exp((-FWHM(i)/(1*pi/pi) + 1i*2*pi*(freq(i))).*time_sample);
%         Z_sig(:,i)=Z_sig(:,i) + (.3.*randn(size(Z_sig(:,i))));
...
%     end
    sub_data_part_temp=sub_data_part;
    for ii=0:RE-1 
        for kk=0: AS-1  
            count_space=0;
            for l = 1:((PE/Ry)/AS)
...
%                D_seg3=D_seg3.*(repmat(conj(amp_basis(met_ind))',size(D_seg3,1),1));
               ...
               sub_ref(:,l)=ref_sig((1+blip_pt + fac2*(l-1-(flag/1)):fac2*(l-(flag/1))));
                rat_ref(:,l)=(angle(sub_ref(:,1)))- angle(abs(sub_ref(:,l)));
...
%                 sub_sig_interp_store_interp(:,l+(kk*(PE/AS/Ry))+(ii*PE/Ry))=ssa(sub_sig_interp_store_interp(:,l+(kk*(PE/AS/Ry))+(ii*PE/Ry)),3);
                P = pseudoinverse(D_seg3_interp,[],'lsqr','tikhonov', {@(x,r) r*normest(D_seg3_interp)*x, 1e-8});
...
%                 fit_param1(:,1)=abs(amp_basis).*exp(1i*(phase_param(l+(kk*(PE/AS/Ry))+(ii*PE/Ry),:)- 1*phas_ref)');
                 for iii=1:model_order
                     if(abs(fit_param1(iii,1))>=1.05*abs(amp_basis(iii)) || T2_flag(iii,l)==1)% && abs(fit_param1(iii,1))<=noise_level)% || T2_flag(iii,l)==1)
                         fit_param1(iii,1)=0;
                     end
                 end
...
%                  sig_sum(((1+blip_pt + fac2*(l-1-(flag/1)):fac2*(l-(flag/1)))))=conj(sub_data_part(:,l+(kk*(PE/AS))+(ii*PE)));
                 ...
                 sig_sum4(1+blip_pt :fac2)=sig_sum3;%./ref_param_rat2(1,l);
                 extrapolated_signal(:,l+(kk*(PE/AS/Ry))+(ii*PE/Ry))=sig_sum2;
...
%     extrapolated_signal3=extrapolated_signal(1:end,:);
    net_blipped_sig_org1(:,1:length(scale_half_temp))=conj(extrapolated_signal)';
%     net_blipped_sig_org1(:,1:size(sub_data_part,1))=(sub_data_part)';
...
%------------------------------------------------------------------------------------------------------------%
    ...
    % SNR_each(9)=[];
    SNR=mean(SNR_each);
    vox_RL=0;%round(RE*RL_off./FOV_RL);
    vox_PE=0;%round(PE*PE_off./FOV_PE);
...
%                       tmp(:,ii)=image_smooth(((tmp(:,ii))),1);%2.*PE./8);
...
%                 end
%                 tmp_zero=zeros(PE*zerofill_value,RE*zerofill_value);
%                 tmp_zero(1:PE*(zerofill_value),1:RE*(zerofill_value))=tmp;
...
%                     tmp(ii,:)=fliplr(tmp(ii,:));
                ...
                for ii=1:PE*zerofill_value
%                      tmp_zero(ii,:)=fliplr(tmp_zero(ii,:));
    %                 tmp(ii,:)=(tmp(:,ii));
                end
                temp=(tmp_zero);
...
for i=1:RE*PE
%     recon_sig_fft=(fftshift(fft(recon_sig(:,i))));
%     recon_sig(:,i)=fftshift(fft((tot_sig(i,:)')));
...
data_value.file_name{1}='example1.mat';
data_value.file_name{2}='example1.mat';
