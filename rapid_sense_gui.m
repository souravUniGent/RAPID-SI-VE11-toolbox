...
for i=1:2
    ...
    data=twix_obj.image;
    try
        pos=twix_obj.hdr.Phoenix.sAdjData.sAdjVolume;
    catch
        data_value.data_error=1;
        return;
    end
    ...
    FullData=permute(reshape(double(twix_obj.image()),[twix_obj.image.NCol twix_obj.image.NCha twix_obj.image.NLin*twix_obj.image.NPhs twix_obj.image.NSet twix_obj.image.NSeg]),[2 1 3 5 4]);
    FullData=sum(FullData,5)./twix_obj.image.NSet;
    firstpoint=mean(conj(FullData(:,1,:)),3);
    channels_scale=squeeze(sqrt(sum(firstpoint.*conj(firstpoint))));
    firstpoint=repmat(firstpoint, [1 twix_obj.image.NCol twix_obj.image.NLin*twix_obj.image.NPhs])/channels_scale;
    FullData = FullData*global_rescale;
    for ll=1:nreceivers
        if twix_obj.image.NSeg>1
    ...
    for ii=0:(RE/Rx)-1
        for kk=0:AS-1
            for l=1:(PE/AS/Ry)
                 ...
                 temp(1:blip_pt)=[];
                 sub_data_part(:,l+(kk*(PE/AS/Ry))+(ii*PE/Ry))=temp;
            end
        end
    end
    % if rev_mode==1
...
%         Z_sig(:,i)=exp((1i*2*pi*(freq(i))).*time_sample);
        Z_sig(:,i)=exp((-FWHM(i)/(4*pi) + 1i*2*pi*(freq(i))).*time_sample);
...
%                     tmp(:,ii)=ssa(tmp(:,ii),2.*PE./8);
                    tmp_real(:,ii)=ssa(real(tmp(:,ii)),2);%2.*PE./8);
                    tmp_imag(:,ii)=ssa(imag(tmp(:,ii)),2);%2.*PE./8);
...
%                tmp_full(1:1:PE,2:2:RE)=0;
               tmp_zero=tmp_full;
               tmp_zero =(fftshift(fft2(((tmp_zero)))));
%                tmp_2=tmp_full_2;
...
%-------------------------------------------------------------------------------------------
f_name=(['sense_map',num2str(PE),'_invivo.mat']);
% load(f_name,'img_image2','img_image2_real','img_image2_imag','img_image2_phase');
load(f_name,['img_image2_',num2str(PE)],['img_image2_real_',num2str(PE)],['img_image2_imag_',num2str(PE)],['img_image2_phase_',num2str(PE)]);
recon_sig_map=eval(['img_image2_',num2str(PE)]);
recon_sig_map_phase=eval(['img_image2_phase_',num2str(PE)]);
% recon_sig_map_2=eval(['img_image2_',num2str(PE)]);
% recon_sig_map_phase_2=eval(['img_image2_phase_',num2str(PE)]);
for lll=1:length(scale_half)
    for ll=1:nreceivers
